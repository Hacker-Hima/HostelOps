...
export default function TechnicianView({ page, isMobile }) {
  ...
  return (
    <div style={{width:'min(900px,100%)',animation:'slideUp 0.3s ease'}}>
      <div style={{background:'var(--bg-card)',border:'1px solid var(--border-subtle)',borderRadius:'var(--radius-xl)',overflow:'hidden',boxShadow:'var(--shadow-float)'}}>
        <div style={{display:'flex',minHeight:640}}>
          <div style={{width:220,background:'var(--bg-sidebar)',borderRight:'1px solid var(--border-subtle)',padding:'24px 14px',display:'flex',flexDirection:'column'}}>
            ...
            <div className="sidebar-profile"><div className="avatar avatar-sm" style={{background:'linear-gradient(135deg,#f97316,#eab308)'}}>⚡</div><div><strong style={{fontSize:11}}>Sarathi Kamal</strong><span>{t('role_technician', 'Electrician')}</span></div></div>
          </div>
          <div style={{flex:1,padding:'24px 28px',overflow:'auto',background:'rgba(3,8,20,0.4)'}}>{renderContent()}</div>
        </div>
      </div>
    </div>
  );
