...
function StudentProfile({ currentUser, tickets, t }) {
  ...
  return (
    <>
      ...
      <button className="btn btn-ghost btn-full" style={{ marginTop:8 }}>⚙️ Edit Profile</button>
    </>
  );
}
    ...
    <div style={{ width:'min(900px,100%)', animation:'slideUp 0.3s ease' }}>
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-subtle)', borderRadius:'var(--radius-xl)', overflow:'hidden', boxShadow:'var(--shadow-float)' }}>
        <div style={{ display:'flex', minHeight:640 }}>
          <div style={{ width:240, background:'var(--bg-sidebar)', borderRight:'1px solid var(--border-subtle)', padding:'24px 16px', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ flex:1, padding:'24px 28px', overflow:'auto', background:'rgba(3,8,20,0.4)' }}>
            {renderPageContent()}
          </div>
        </div>
      </div>
