...
export async function connectDB() {
  try {
    ...
    // If primary URI fails and isn't local, attempt graceful fallback to local MongoDB
    if (
      ...
      try {
        const localConn = await mongoose.connect('mongodb://127.0.0.1:27017/hostelops', {
          serverSelectionTimeoutMS: 3000,
        });
        isConnected = true;
        console.log(`✅ MongoDB Connected (Local Fallback): ${localConn.connection.host}:${localConn.connection.port}/${localConn.connection.name}`);
        await seedInitialData();
        return;
      } catch (fallbackError) {
        // Fallback also failed, proceed to log warning below
      }
    }
...
export async function seedInitialData() {
  try {
    console.log('🌱 Checking & initializing MongoDB data...');
    ...
    // 3. Seed Comments
    if ((await TicketComment.countDocuments()) === 0) {
    ...
    // 4. Seed Ratings
    if ((await TicketRating.countDocuments()) === 0) {
    ...
    if ((await StaffRequest.countDocuments()) === 0) {
